#!/usr/bin/env python3
"""
DITA Compliance Report Generator — OADP / OpenShift API for Data Protection

Runs all standard DITA migration checks against the OADP section of the
openshift-docs repository and writes results to an Excel workbook with one
sheet per check.

The vale check uses asciidoctor-list-content to expand includes, so module
files referenced by OADP assemblies are checked even if they live outside the
backup_and_restore/application_backup_and_restore directory.

Requirements:
    pip install openpyxl
    gem install asciidoctor-list-content

Usage (run from the root of the openshift-docs repo):
    python3 dita-report-oadp.py
    python3 dita-report-oadp.py --vale ~/dita.ini --out oadp-dita-report.xlsx
"""
...
OADP_DIR = "backup_and_restore/application_backup_and_restore"

# Replicates exactly:
#   { find <dir> -type f -name '*.adoc';
#     find <dir> -type f -name '*.adoc' | xargs -n 1 list-content; }
#   | sort -u | grep -v '_attributes/' | xargs -n 1 -P 8 vale ...
FIND_CMD = (
    f"{{ find {OADP_DIR} -type f -name '*.adoc';"
    f" find {OADP_DIR} -type f -name '*.adoc' | xargs -n 1 list-content; }}"
    f" | sort -u | grep -v '_attributes/'"
)

GREP_TARGETS = OADP_DIR
...
def check_dependencies():
    result = subprocess.run(
        "list-content --version", shell=True, capture_output=True, text=True
    )
    if result.returncode != 0:
        sys.exit(
            "ERROR: list-content not found.\n"
            "Install it with: gem install asciidoctor-list-content"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="DITA compliance report for OADP / OpenShift API for Data Protection"
    )
    parser.add_argument("--vale", default="~/dita.ini", help="Path to vale config")
    parser.add_argument("--out", default="oadp-dita-report.xlsx", help="Output file")
    args = parser.parse_args()

    if not Path(OADP_DIR).is_dir():
        sys.exit(
            f"ERROR: {OADP_DIR} not found — "
            "run this from the root of the openshift-docs repo."
        )

    check_dependencies()

    print("Repo:        OADP / OpenShift API for Data Protection (openshift-docs)")
    print(f"Scope:       {OADP_DIR} (+ included modules via list-content)")
    ...
    # Note: grep covers OADP_DIR only; included modules outside this
    # directory are checked by vale (via list-content) but not by grep.
    # ------------------------------------------------------------------
    ...
    sheets_meta.append({
        ...
        "count": len(cond_rows),
        "notes": f"Searched {OADP_DIR} only. ifeval: {len(ifeval_lines)}, non-context ifdef/ifndef: {len(ifdefndef_non_context)}",
    })
