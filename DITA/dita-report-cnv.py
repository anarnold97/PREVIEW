#!/usr/bin/env python3
"""
DITA Compliance Report Generator — CNV / OpenShift Virtualization

Runs all standard DITA migration checks against virt/ and modules/virt-*.adoc
in the openshift-docs repository and writes results to an Excel workbook with
one sheet per check.
...
Usage (run from the root of the openshift-docs repo):
    python3 dita-report-cnv.py
    python3 dita-report-cnv.py --vale ~/dita.ini --out cnv-dita-report.xlsx
"""
...
FIND_CMD = "find virt/ modules/virt-*.adoc -type f"
GREP_TARGETS = "virt/ modules/virt-*.adoc"
...
def main():
    parser = argparse.ArgumentParser(
        description="DITA compliance report for CNV / OpenShift Virtualization"
    )
    parser.add_argument("--vale", default="~/dita.ini", help="Path to vale config")
    parser.add_argument("--out", default="cnv-dita-report.xlsx", help="Output file")
    args = parser.parse_args()

    if not Path("virt").is_dir():
        sys.exit("ERROR: virt/ directory not found — run this from the root of the openshift-docs repo.")

    print(f"Repo:        CNV / OpenShift Virtualization (openshift-docs — virt/)")
