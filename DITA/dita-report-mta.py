#!/usr/bin/env python3
"""
DITA Compliance Report Generator — MTA / Migration Toolkit for Applications

Runs all standard DITA migration checks against assemblies/ and docs/topics/
in the mta-documentation repository and writes results to an Excel workbook
with one sheet per check.

Requirements:
    pip install openpyxl

Usage (run from the root of the mta-documentation repo):
    python3 dita-report-mta.py
    python3 dita-report-mta.py --vale ~/dita.ini --out mta-dita-report.xlsx
"""
...
FIND_CMD = (
    "find assemblies/ docs/topics/ -type f -name '*.adoc'"
    " -not -name 'document-attributes.adoc'"
)
GREP_TARGETS = "assemblies/ docs/topics/"
...
def main():
    parser = argparse.ArgumentParser(
        description="DITA compliance report for MTA / Migration Toolkit for Applications"
    )
    parser.add_argument("--vale", default="~/dita.ini", help="Path to vale config")
    parser.add_argument("--out", default="mta-dita-report.xlsx", help="Output file")
    args = parser.parse_args()

    if not Path("assemblies").is_dir() or not Path("docs/topics").is_dir():
        sys.exit("ERROR: assemblies/ or docs/topics/ not found — run this from the root of the mta-documentation repo.")

    print("Repo:        MTA / Migration Toolkit for Applications (mta-documentation)")
