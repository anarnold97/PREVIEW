#!/usr/bin/env python3
"""
DITA Compliance Report Generator — RHOSO / Red Hat OpenStack

Runs all standard DITA migration checks against assemblies/, common/, and
doc-*/ directories in the RHOSO documentation repository and writes results
to an Excel workbook with one sheet per check.

Requirements:
    pip install openpyxl

Usage (run from the root of the RHOSO docs repo):
    python3 dita-report-rhoso.py
    python3 dita-report-rhoso.py --vale ~/dita.ini --out rhoso-dita-report.xlsx
"""
...
# Replicates exactly:
#   find assemblies/ common/ doc-* -type f -name '*.adoc'
#     -not -name 'master.adoc'
#     -not -name 'stf-attributes.adoc'
#     -not -path '*/global/*attributes.adoc'
#   | sed -e 's/ /\\&/'
#   | xargs -n 1 -P 8 vale --config ~/dita.ini --output line
#
# Raw string used so Python does not interpret the backslash in the sed expression.
FIND_CMD = (
    r"find assemblies/ common/ doc-* -type f -name '*.adoc'"
    r" -not -name 'master.adoc'"
    r" -not -name 'stf-attributes.adoc'"
    r" -not -path '*/global/*attributes.adoc'"
    r" | sed -e 's/ /\\&/'"
)

# grep targets — shell expands doc-*/ to all matching directories
GREP_TARGETS = "assemblies/ common/ doc-*/"
...
def sync_with_main() -> bool:
    """Fetch origin and rebase onto the remote default branch before running checks."""
    ...
    # Detect the remote default branch: try origin/HEAD, then main, then master
    code, remote_ref, _ = _git("symbolic-ref --short refs/remotes/origin/HEAD")
    if code != 0:
...
def main():
    parser = argparse.ArgumentParser(
        description="DITA compliance report for RHOSO / Red Hat OpenStack"
    )
    parser.add_argument("--vale", default="~/dita.ini", help="Path to vale config")
    parser.add_argument("--out", default="rhoso-dita-report.xlsx", help="Output file")
    args = parser.parse_args()

    if not Path("assemblies").is_dir() or not Path("common").is_dir():
        sys.exit(
            "ERROR: assemblies/ or common/ not found — "
            "run this from the root of the RHOSO docs repo."
        )

    print("Repo:        RHOSO / Red Hat OpenStack (rhoso)")
    print(f"Scope:       assemblies/ common/ doc-*/")
