#!/usr/bin/env python3
"""
CQA 2.1 + JTBD validation for AsciiDoc documentation.

Standalone script: run from any directory. Validates .adoc files against
DITA migration standards (CQA 2.1) and optionally checks alignment with
a JTBD (Jobs-to-be-Done) mapping file.

Usage:
  # Validate repo (default: current directory)
  python3 cqa_jtbd_validate.py
  python3 cqa_jtbd_validate.py --docs-dir /path/to/mta-documentation

  # JTBD coverage only (paths listed in jtbd-mapping.yaml)
  python3 cqa_jtbd_validate.py --docs-dir /path/to/repo --jtbd-only

  # List files that need shortdesc fixes
  python3 cqa_jtbd_validate.py --docs-dir /path/to/repo --fix-shortdesc

  # Machine-readable summary
  python3 cqa_jtbd_validate.py --docs-dir /path/to/repo --json

Optional: PyYAML (pip install pyyaml) for JTBD mapping. Without it, only CQA checks run.
...
# -----------------------------------------------------------------------------
# Optional dependency: PyYAML for JTBD mapping (script runs without it)
# -----------------------------------------------------------------------------
try:
    import yaml
except ImportError:
...
# Allowed content types in _content-type / _mod-docs-content-type
CONTENT_TYPES = {"CONCEPT", "PROCEDURE", "REFERENCE", "ASSEMBLY", "SNIPPET"}
# Topic types that require a short description
...
# Self-referential phrases not allowed in shortdesc (CQA)
...
def load_jtbd_mapping(mapping_path: Path) -> dict[str, Any] | None:
    """
    Load JTBD mapping from a YAML file.
...
def collect_mapped_paths(jtbd: dict[str, Any], repo_root: Path) -> set[Path]:
    """
    Build the set of repo-relative paths referenced in the JTBD mapping
    (assemblies and topics per job). Used for --jtbd-only reporting.
    """
    out: set[Path] = set()
...
def get_content_type(content: str) -> str | None:
    """Extract _content-type or _mod-docs-content-type attribute from AsciiDoc source."""
...
def get_shortdesc_paragraph(content: str) -> tuple[str | None, int, int]:
    """
    Find the single short-description paragraph after [role="_abstract"] (CQA: one paragraph).
    Returns (paragraph_text, start_offset, end_offset) or (None, -1, -1) if no abstract block.
    """
    ...
    start = m.end()
    rest = content[start:]
    # Skip past newlines to start of first line of the paragraph
    i = 0
    while i < len(rest) and rest[i : i + 1] in ("\n", "\r"):
        i += 1
    block_start = i
    # First paragraph only: stop at first blank line
    lines: list[str] = []
    while i < len(rest):
        line_end = rest.find("\n", i)
        if line_end == -1:
...
def check_blank_line_after_title(content: str) -> tuple[bool, str]:
    """CQA: require a blank line between the level-0 title and [role="_abstract"]."""
...
def check_shortdesc_length(text: str) -> tuple[bool, str]:
    """CQA: short description must be between 50 and 300 characters."""
    if not text:
...
def check_self_referential(text: str) -> tuple[bool, str]:
    """CQA: short description must not use self-referential phrases (e.g. 'this document')."""
    if not text:
        return True, ""
...
# Line patterns that are allowed between include:: statements in assemblies (CQA)
_ALLOWED_BETWEEN_INCLUDES = (
    re.compile(r"^\s*//"),               # comments
    re.compile(r"^\s*ifdef::"),          # conditionals
    ...
    re.compile(r"^\s*={2,}\s+"),             # section heading
...
def check_assembly_no_text_between_includes(content: str) -> tuple[bool, list[str]]:
    """
    CQA: assembly files must not have narrative content between include statements.
    Conditionals, attribute lines, and section headings after includes are allowed.
...
def count_procedure_steps(content: str) -> int:
    """Count numbered procedure steps (e.g. . 1. Step one)."""
...
def validate_module(
    ...
    """
    Run CQA 2.1 checks on a single topic/module.
    Returns a list of (check_name, message) for each failure.
    """
    ...
    # Short description requirements for modules and assemblies
    ...
    # Procedure-specific: step count and prerequisites
    if content_type == "PROCEDURE":
        ...
        if RE_PREREQUISITES.search(content):
            pass  # Could add prerequisite item count check
...
def validate_assembly(path: Path, content: str) -> list[tuple[str, str]]:
    """
    Run CQA checks specific to assembly files (content-type ASSEMBLY,
    no text between includes), then run shared module shortdesc checks.
    """
    errors: list[tuple[str, str]] = []
...
def scan_adoc_files(base: Path) -> list[Path]:
    """Return sorted list of all .adoc files under base (excluding 'website' is done in main)."""
    return sorted(base.rglob("*.adoc"))


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate AsciiDoc docs against CQA 2.1 and optional JTBD mapping (standalone)."
    )
    parser.add_argument(
        "--docs-dir",
        type=Path,
        default=Path.cwd(),
        help="Repository root containing .adoc files (default: current directory)",
    )
    parser.add_argument(
        "--mapping",
        type=Path,
        default=None,
        help="Path to jtbd-mapping.yaml (default: <docs-dir>/docs/jtbd-mapping.yaml)",
    )
    parser.add_argument(
        "--jtbd-only",
        action="store_true",
        help="Only report JTBD mapping coverage (paths in mapping); no CQA checks",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    repo = args.docs_dir.resolve() if args.docs_dir else Path.cwd().resolve()
    mapping_path = args.mapping or (repo / "docs" / "jtbd-mapping.yaml")

    if not repo.is_dir():
        print("Error: --docs-dir is not a directory:", repo, file=sys.stderr)
        return 1

    jtbd = load_jtbd_mapping(mapping_path)
    mapped_paths = collect_mapped_paths(jtbd, repo) if jtbd else set()

    # All .adoc under repo except under any 'website' directory
    ...
    for path in all_adoc:
        ...
        # Assemblies: file name contains "assembly_" and "assembly"
